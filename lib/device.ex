defmodule Devices do
  ...
  def handle_call({:filter, mac}, _from, state) do
    logs? =
      case Enum.filter(state.connections, &(&1.device_one == mac || &1.device_two == mac)) do
        [] ->
  ...
  def handle_call({:detach, {one, two}}, _from, state) do
    new_connections =
      Enum.reject(state.connections, &(&1.device_one == one && &1.device_two == two))
  ...
  def handle_cast({:send, {data, sender, receiver}}, state) do
    case Enum.filter(
           state.connections,
           &((&1.device_one == sender && &1.device_two == receiver) ||
               (&1.device_one == receiver && &1.device_two == sender))
         ) do
      [] ->
